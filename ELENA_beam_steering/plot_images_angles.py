...
def load_data(first_run:int=492483,last_run:int=492651,bad_runs:list[int]|None=None,download_with_ALPACA:bool=False,filename:str="PbarsOnHHMCP_angles.parquet",verbose:str=False)->pl.DataFrame:
    ...
    offsets = pl.DataFrame({
        'run':[run for run in range(492483,492652)],
        'V_angle_set':[v for v in np.linspace(-3,3,13) for _ in np.linspace(-3,3,13)],
        'H_angle_set':[h for _ in np.linspace(-3,3,13) for h in np.linspace(-3,3,13)]
    })
    data = data.join(offsets.lazy(),on='run').with_columns(((pl.col("ELENA_H_angle_mrad")==pl.col("H_angle_set")) & (pl.col("ELENA_V_angle_mrad")==pl.col("V_angle_set"))).alias("values set")).filter(pl.col("run").is_between(first_run,last_run)).collect()
...
def plot_all_images(data:pl.DataFrame):
    ...
    V_angle_to_plot = np.flip(np.arange(-3.0,3.1,0.5))
    H_angle_to_plot = np.arange(-3.0,3.1,0.5)
    
    norm = mcolors.Normalize(vmin=data.select(pl.col("MCP_img").arr.min().min()).item(),vmax=data.select(pl.col("MCP_img").arr.max().max()).item())
    fig,axes = plt.subplots(len(H_angle_to_plot),len(V_angle_to_plot),squeeze=False,sharex=True,sharey=True,figsize=FIGSIZE)
    for ax in axes.flatten():
        ax.set_xticks([])
        ax.set_yticks([])
        
    data_filter = data.filter(pl.col("V_angle_set").is_in(V_angle_to_plot) & pl.col("H_angle_set").is_in(H_angle_to_plot))
    for run in pl.Series(data_filter.select("run")).to_list():
        run_data = data_filter.filter(pl.col("run")==run)
        print(run)
        H_angle = run_data.select('ELENA_H_angle_mrad').item()
        H_angle_set = run_data.select('H_angle_set').item()
        V_angle = run_data.select('ELENA_V_angle_mrad').item()
        V_angle_set = run_data.select('V_angle_set').item()
        print(f"\tH angle = {H_angle} should be {H_angle_set} ({run_data.select('values set').item()})")
        print(f"\tV angle = {V_angle} should be {V_angle_set} ({run_data.select('values set').item()})")
        
        col = np.where(H_angle_to_plot == H_angle_set)[0][0] # int((run_data.select("V_angle_set").item() + 10) % 21)
        row = np.where(V_angle_to_plot == V_angle_set)[0][0] # int((run_data.select("H_angle_set").item() + 10) % 21)
        ...
        ax.set_xlabel(H_angle,color='black' if H_angle==H_angle_set else 'red')
        ax.set_ylabel(V_angle,color='black' if V_angle==V_angle_set else 'red')
        
        if col == len(H_angle_to_plot)-1:
            ax2 = ax.twinx()
            ax2.set_yticks([])
            ax2.set_ylabel(V_angle_set)
        
        if row == 0:
            ax.set_title(H_angle_set)
    
    ax = fig.add_subplot(111, frameon=False)
    ax.set_xlabel("H angle set",labelpad=15)
    ax.set_ylabel("V angle set",labelpad=15)
    ax.set_xticks([])
    ax.set_yticks([])
    ax2 = fig.add_subplot(111, frameon=False)
    ax2.xaxis.set_label_position("top")
    ax2.yaxis.set_label_position("right")
    ax2.set_xlabel("H angle requested",labelpad=15)
    ax2.set_ylabel("V angle requested",labelpad=15)
    ax2.set_xticks([])
    ax2.set_yticks([])
    fig.tight_layout()
    # fig.subplots_adjust(left=0.1, top=0.9,bottom=0.1,right=0.9)
    fig.savefig(os.path.join(os.path.dirname(__file__),"plots",'angle_scan_results.png'))
    plt.show()
...
def plot_single_image(data:pl.DataFrame,run:int,showfig:bool=False):
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_title(f'{run}, V_ANGLE={run_data.select("ELENA_V_angle_mrad").item()} (requested {run_data.select("V_angle_set").item()}), H_ANGLE={run_data.select("ELENA_H_angle_mrad").item()}  (requested {run_data.select("H_angle_set").item()})')
    
    fig.tight_layout()
    fig.savefig(os.path.join(os.path.dirname(__file__),"plots",f'{run}-V_angle={run_data.select("V_angle_set").item()}-H_angle={run_data.select("H_angle_set").item()}.png'))
    if showfig:
        plt.show()
    else:
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    for run in range(492483,492652):
        print(run) 
        try:
